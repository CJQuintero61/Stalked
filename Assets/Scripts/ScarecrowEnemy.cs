...
public class ScarecrowEnemy : MonoBehaviour, IDamageDealer
{
    ...
    void Update()
    {
        ...
        {
            CheckActivation();
            FreezeEnemy(); // Stay still until the collar is found
            return;
        }
    ...
    {
        if (GameManager.Instance != null && GameManager.Instance.hasCollar)
        {
            isActivated = true;
            Debug.Log("Scarecrow: I have felt the collar. Starting chase.");
        }
    }
    ...
    public void ConfigureSwitchingTargets(List<GameObject> decoys, Transform playerTransform, Camera camera)
    {
        ...
        if (playerCamera == null) playerCamera = camera;
        ...
        if (decoys != null)
        {
            ...
            {
                if (decoy != null) decoySpots.Add(decoy);
            }
        }
    ...
    {
        if (!enableScarecrowSwitching || decoySpots.Count <= 1) return;
        if (switchPauseTimer > 0f)
        {
    ...
    bool TryGetNextDecoy(out GameObject nextDecoy)
    {
        ...
        {
            if (IsValidSwitchTarget(decoy, false)) strictValidDecoys.Add(decoy);
            if (IsValidSwitchTarget(decoy, true)) relaxedValidDecoys.Add(decoy);
        }
    ...
    {
        if (decoy == null || decoy == currentDecoySpot) return false;

        if (player != null)
        {
            float distanceFromPlayer = Vector3.Distance(player.position, decoy.transform.position);
            if (distanceFromPlayer < minSwitchDistanceFromPlayer) return false;
            if (maxSwitchDistanceFromPlayer > 0f && distanceFromPlayer > maxSwitchDistanceFromPlayer) return false;
        }

        if (!ignoreVisibilityCheck && avoidSwitchingIntoView && IsDecoyVisibleToPlayer(decoy)) return false;
    ...
    {
        if (playerCamera == null || decoy == null) return false;
        ...
        if (distance <= 0.01f || distance > switchSightDistance) return false;

        Vector3 direction = toTarget / distance;
        float dot = Vector3.Dot(playerCamera.transform.forward, direction);
        if (dot < switchViewDotThreshold) return false;
    ...
    bool CanPlayerSeeEnemy()
    {
        ...
        if (IsSightBlockedByCorn(sightStart, targetPoint)) return false;
    ...
    {
        if (!cornBlocksSight) return false;
    ...
    bool IsCornColliderBlockingSight(Vector3 sightStart, Vector3 sightEnd)
    {
        ...
        direction /= distance;
        RaycastHit[] hits = Physics.RaycastAll(sightStart, direction, distance, cornCollisionMask, QueryTriggerInteraction.Collide);

        foreach (RaycastHit hit in hits)
        {
            if (hit.collider != null && IsCornObject(hit.collider.transform)) return true;
        }
        return false;
    }
    ...
    bool IsCornRendererBlockingSight(Vector3 sightStart, Vector3 sightEnd)
    {
        ...
        foreach (Renderer cornRenderer in cornRenderers)
        {
            ...
            if (bounds.Contains(sightStart)) return true;
            if (bounds.IntersectRay(sightRay, out float hitDistance) && hitDistance <= distance) return true;
        }
        return false;
    }
    ...
    bool IsCornTerrainTreeBlockingSight(Vector3 sightStart, Vector3 sightEnd)
    {
        ...
        {
            if (DistanceToSegmentXZSquared(treePosition, sightStart, sightEnd) <= blockingRadiusSqr) return true;
        }
        return false;
    }
    ...
    bool IsCornTerrainDetailBlockingSight(Vector3 sightStart, Vector3 sightEnd)
    {
        ...
        for (int i = 1; i < sampleCount; i++)
        {
            ...
            if (IsOnCornTerrainDetail(samplePoint)) return true;
        }
        return false;
    }
    ...
    bool IsOnCornTerrainDetail(Vector3 position)
    {
        foreach (TerrainCornDetailLayer cornDetailLayer in cornDetailLayers)
        {
            ...
            if (localPosition.x < 0f || localPosition.z < 0f || localPosition.x > terrainData.size.x || localPosition.z > terrainData.size.z) continue;
            int detailX = Mathf.Clamp(Mathf.FloorToInt((localPosition.x / terrainData.size.x) * terrainData.detailWidth), 0, terrainData.detailWidth - 1);
            int detailZ = Mathf.Clamp(Mathf.FloorToInt((localPosition.z / terrainData.size.z) * terrainData.detailHeight), 0, terrainData.detailHeight - 1);
            int[,] details = terrainData.GetDetailLayer(detailX, detailZ, 1, 1, cornDetailLayer.layerIndex);
            if (details[0, 0] > 0) return true;
        }
        return false;
    }
    ...
    void CacheCornSightBlockers()
    {
        ...
        if (!cornBlocksSight) return;
        foreach (Renderer sceneRenderer in FindObjectsByType<Renderer>(FindObjectsSortMode.None))
        {
            if (sceneRenderer != null && IsCornObject(sceneRenderer.transform)) cornRenderers.Add(sceneRenderer);
        }
        foreach (Terrain terrain in Terrain.activeTerrains)
        {
            ...
            for (int i = 0; i < detailPrototypes.Length; i++)
            {
                ...
                if (IsCornName(dp.prototype != null ? dp.prototype.name : null) || IsCornName(dp.prototypeTexture != null ? dp.prototypeTexture.name : null))
    ...
    void CacheCornTerrainTrees(Terrain terrain)
    {
        ...
        HashSet<int> cornIdx = new HashSet<int>();
        for (int i = 0; i < treePrototypes.Length; i++)
        {
            if (treePrototypes[i].prefab != null && IsCornName(treePrototypes[i].prefab.name)) cornIdx.Add(i);
        }
        if (cornIdx.Count == 0) return;
        foreach (TreeInstance tree in terrainData.treeInstances)
        {
            if (!cornIdx.Contains(tree.prototypeIndex)) continue;
            Vector3 worldPos = Vector3.Scale(tree.position, terrainData.size);
            cornTerrainTreePositions.Add(terrain.transform.position + worldPos);
        }
    }

    bool IsCornObject(Transform t)
    {
        while (t != null) { if (IsCornName(t.name)) return true; t = t.parent; }
        return false;
    }

    bool IsCornName(string n)
    {
        if (string.IsNullOrEmpty(n)) return false;
        foreach (string k in cornNameKeywords)
        {
            if (!string.IsNullOrEmpty(k) && n.IndexOf(k, System.StringComparison.OrdinalIgnoreCase) >= 0) return true;
        }
        return false;
    }
    ...
    {
        bool warped = false;
        if (agent != null && agent.enabled) warped = agent.Warp(position);
        if (!warped) transform.position = position;
    ...
    void CachePlayerHealth()
    {
        ...
        if (playerHealth == null) playerHealth = player.GetComponent<PlayerHealth>() ?? player.GetComponentInChildren<PlayerHealth>();
    }
    ...
    {
        if (!IsPlayerWithinAttackRange()) return false;
        FacePlayer();
        if (agent != null && agent.isOnNavMesh) { agent.isStopped = true; agent.ResetPath(); }
        if (animator != null) animator.SetBool("IsMoving", false);
        CachePlayerHealth();
        if (attackCooldownTimer > 0f || playerHealth == null) return true;
    ...
    bool IsPlayerWithinAttackRange()
    {
        ...
        Vector3 p = player.position; Vector3 e = transform.position;
        p.y = 0f; e.y = 0f;
        return Vector3.Distance(e, p) <= attackRange;
    }
    ...
    void FacePlayer()
    {
        ...
        transform.rotation = Quaternion.RotateTowards(transform.rotation, rot, attackTurnSpeed * Time.deltaTime);
    }

    void UpdateFootsteps()
    {
        float dist = Vector3.Distance(transform.position, lastFootstepPosition);
        lastFootstepPosition = transform.position;
        if (agent == null || audioSource == null || footstepSounds == null || footstepSounds.Length == 0) return;
        bool moving = !agent.isStopped && agent.velocity.magnitude > minimumFootstepSpeed;
        if (!moving) return;
        footstepDistanceTravelled += dist;
        if (footstepDistanceTravelled < stepDistance) return;
        PlayFootstep();
        footstepDistanceTravelled = 0f;
    }

    void ResetFootstepTracking() { footstepDistanceTravelled = 0f; lastFootstepPosition = transform.position; }
    ...
    void ChasePlayer()
    {
        ...
        if (needsDestinationRefresh || !agent.hasPath || chaseTimer <= 0f)
        {
